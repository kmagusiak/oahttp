...
class Request:
    ...
    def _receive_data(self, buf: ReadBuffer) -> None:
        ...
        if not self.host:
            raise HttpSyntaxError("missing host header")  # XXX other exception?
        # set the body
        if self.headers.get(b'transfer-encoding'):
            # XXX if content-length, server must close
            # XXX supporting only chunked, raise 501 otherwise
...
class RequestBody:
    ...
    async def wait(self):  # XXX Awaitable?
        ...
...
class ChunkedBodyReceiver(RequestBody):
    ...
    def receive_data(self, buf: ReadBuffer):
        ...
        while self.__reading == 'chunk':
            ...
            size = m.group(0)
            # XXX parse and validate size (hexa)
            if size:
                self.__receiver.receive_exactly(buf, size)  # XXX
...
class BodyFileReceiver(RequestBody):
    def __init__(self, expected_size):
        ...
        self.__fd = tempfile.SpooledTemporaryFile()
