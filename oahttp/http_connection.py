...
class HttpConnection(asyncio.BufferedProtocol):
    ...
    def buffer_updated(self, nbytes):
        ...
        try:
            dispatch = eager(loop, self.strategy.dispatcher(request), context=context)
        except Exception as e:
            from .response import Response
            if isinstance(e, Response):
                expect_body = False
                error_response = e
                async def async_error() -> Response:
                    return error_response
                dispatch = eager(loop, async_error(), context=context)
            else:
                self.abort(e)
                return
        try:
    ...
    async def _response_callback(self, task: asyncio.Task[Response]):
    ...
    def abort(self, response: Response | Exception | None = None):
        ...
        if response is not None:  # TODO and not already started sending
            if isinstance(response, Exception):
                ...
                response = self.strategy.wrap_error(self.request, response)
